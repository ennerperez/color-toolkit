...
namespace Toolkit
{
    public static partial class Extensions
    {
        ...
        public static Color FromHsb(this Color color, int alpha, float hue, float saturation, float brightness)
        {
            int d = 1530;
            int mid;
            int max = (int)Math.Round(brightness * 255);
            int min = (int)Math.Round((1.0 - saturation) * (brightness / 1.0) * 255);
            double q = (double)(max - min) / 255;

            if (hue >= 0 && hue <= (double)1 / 6)
            {
                mid = (int)Math.Round(((hue - 0) * q) * d + min);
                color = Color.FromArgb(alpha, max, mid, min);
            }

            if (hue <= (double)1 / 3)
            {
                mid = (int)Math.Round(-((hue - (double)1 / 6) * q) * d + max);
                color = Color.FromArgb(alpha, mid, max, min);
            }

            if (hue <= 0.5)
            {
                mid = (int)Math.Round(((hue - (double)1 / 3) * q) * d + min);
                color = Color.FromArgb(alpha, min, max, mid);
            }

            if (hue <= (double)2 / 3)
            {
                mid = (int)Math.Round(-((hue - 0.5) * q) * d + max);
                color = Color.FromArgb(alpha, min, mid, max);
            }

            if (hue <= (double)5 / 6)
            {
                mid = (int)Math.Round(((hue - (double)2 / 3) * q) * d + min);
                return Color.FromArgb(alpha, mid, min, max);
            }

            if (hue <= 1.0)
            {
                mid = (int)Math.Round(-((hue - (double)5 / 6) * q) * d + max);
